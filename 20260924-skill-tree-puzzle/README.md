...
│   │   └── simulate.js  # ビルドと敵から勝敗を決め、お題の敵すべてに勝てたかを見る
│   ├── data/
│   │   ├── tree.js        # スキルツリー（30ノード）
│   │   └── challenges.js  # お題（敵の組と配布 pt）
│   └── ui/
│       ├── tree-view.js      # ツリーの描画とタップの通知
│       ├── panel.js          # お題の選択と情報・残り pt・挑戦と全リセットのボタン
...
1. お題を選び、ボスの HP・攻撃・反撃・ターン上限・軽減率を見る。
   ボスが複数いるお題では、ボスごとに1行ずつ出る
2. ノードをタップして取得する。取得済みのノードをもう一度タップすると外せる。
   組んだビルドの HP・攻撃・防御・属性は、お題の下に出る
3. 「挑戦」を押し、戦闘ログで勝敗を確かめる。
   ボスが複数いるときは、ボスごとの勝敗がタブに並び、タブを押すとそのボスのログに切り替わる
4. 負けたら、ログから理由を読んで組み直す

組んだビルドは、ノードを取得・解除するたびに、お題ごとにブラウザへ保存されます。
お題を切り替えても、タブを閉じても、戻ってきたときに続きから組めます。
次に開いたときは、最後に選んでいたお題から始まります。
